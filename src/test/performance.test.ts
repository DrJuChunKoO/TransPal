...
function generateLargeMarkdown(size: number): string {
  const headings = Array(5).fill(0).map((_, i) => `${'#'.repeat(i+1)} Heading ${i+1}\n\n`).join('');
  const paragraphs = Array(size).fill(0).map((_, i) => `This is paragraph ${i+1} with **bold** and *italic* text.\n\n`).join('');
  const lists = Array(Math.floor(size/5)).fill(0).map((_, i) => `- List item ${i+1}\n`).join('');
  const code = '```javascript\n' + Array(Math.floor(size/10)).fill(0).map((_, i) => `// Line ${i+1}\nconst value${i+1} = ${i+1};\n`).join('') + '```\n\n';
...
describe('Performance Tests', () => {
  ...
  describe('Markdown Processing Performance', () => {
    it('should process small markdown content quickly', () => {
      ...
      // Small markdown should process in under 50ms (adjusted for real-world performance)
  ...
  describe('Memory Usage', () => {
    ...
    it('should manage memory efficiently with markdown cache', () => {
      ...
      const contents = Array(20).fill(0).map((_, i) => generateLargeMarkdown(5 + (i % 10)));
